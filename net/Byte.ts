...
export class Byte {
	...
	public get buffer(): ArrayBuffer {
		const rstBuffer: ArrayBuffer = this._d_.buffer;
	...
	public getFloat32Array(start: number, len: number): any {
		...
		const v: Float32Array = new Float32Array(this._d_.buffer.slice(start, end));
		this._pos_ = end;
		return v;
	}
	...
	public getUint8Array(start: number, len: number): Uint8Array {
		...
		const v: Uint8Array = new Uint8Array(this._d_.buffer.slice(start, end));
		this._pos_ = end;
		return v;
	}
	...
	public getInt16Array(start: number, len: number): any {
		...
		const v: Int16Array = new Int16Array(this._d_.buffer.slice(start, end));
		this._pos_ = end;
		return v;
	}
	...
	public getFloat32(): number {
		...
		const v: number = this._d_.getFloat32(this._pos_, this._xd_);
		this._pos_ += 4;
		return v;
	}
	...
	public getFloat64(): number {
		...
		const v: number = this._d_.getFloat64(this._pos_, this._xd_);
		this._pos_ += 8;
		return v;
	}
	...
	public getInt32(): number {
		...
		const float: number = this._d_.getInt32(this._pos_, this._xd_);
		this._pos_ += 4;
		return float;
	}
	...
	public getUint32(): number {
		...
		const v: number = this._d_.getUint32(this._pos_, this._xd_);
		this._pos_ += 4;
		return v;
	}

	/**
	 * 从字节流的当前字节偏移量位置处读取一个 int64 值。
	 * @return int64 值。
	 */
	...
	/**
	 * 在字节流的当前字节偏移量位置处写入指定的 int64 值。
	 * @param	value	需要写入的 int64 值。
	 */
	...
	public getInt16(): number {
		...
		const us: number = this._d_.getInt16(this._pos_, this._xd_);
		this._pos_ += 2;
		return us;
	}
	...
	public getUint16(): number {
		...
		const us: number = this._d_.getUint16(this._pos_, this._xd_);
		this._pos_ += 2;
		return us;
	}
	...
	/**
	 * 在字节流的当前字节偏移量位置处写入指定的 Int8 值。
	 * @param	value	需要写入的 Int8 值。
	 */
	...
	public writeUTFString(value: string): void {
		const tPos: number = this.pos;
		this.writeUint16(1);
		this.writeUTFBytes(value);
		const dPos: number = this.pos - tPos - 2;
		if (dPos >= 65536) {
	...
	public readUTFBytes(len: number = -1): string {
		if (len == 0) return "";
		const lastBytes: number = this.bytesAvailable;
	 ...
	 */
	public writeArrayBuffer(arraybuffer: any, offset: number = 0, length: number = 0): void {
	 ...
	 * @param	length		从 Arraybuffer 对象写入到 Byte 对象的长度（以字节为单位）
	 */
	public writeBytes(arraybuffer: any, offset: number = 0, length: number = 0): void {
		if (offset < 0 || length < 0) throw "writeArrayBuffer error - Out of bounds";
		if (length == 0) length = arraybuffer.byteLength - offset;
		//$ALERT 这里会分配用户指定的内存空间，这可能导致分配多余的内存空间，甚至导致内存溢出。应该进行有效性检查。如果用户想要分配多余的空间，应该使用set length。
		this.ensureWrite(this._pos_ + length);
