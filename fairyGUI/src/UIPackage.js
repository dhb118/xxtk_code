import { Asset, AssetManager, AudioClip, BitmapFont, BufferAsset, dragonBones, ImageAsset, path, Rect, resources, Size, sp, SpriteFrame, Vec2 } from "cc";
...
export class UIPackage {
    ...
    loadPackage(buffer, path) {
        ...
        for (i = 0; i < cnt; i++) {
            const dependId = buffer.readS();
            const dependName = buffer.readS();
            this._dependencies.push({ id: dependId, name: dependName });
        }
        if (ver2) {
            cnt = buffer.readShort();
            if (cnt > 0) {
    ...
    dispose() {
        ...
        for (var i = 0; i < cnt; i++) {
            ...
            if (pi.file) {
                const asset = resources.get(pi.file);
                asset.decRef(true);
            }
        }
    }
